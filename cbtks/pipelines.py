...
class CcbtksPipeline:

    # Database setup
    conn = sqlite3.connect('cbtks.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `cbtks`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM cbtks WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO cbtks (date, title, link, content)"
